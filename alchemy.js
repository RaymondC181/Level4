...
{
    var idx = parseInt(organismForm.orgse.value)-1;

    if(idx<0 || idx >= myOrganisms.length)
    {
        return;
    
    if(firstSelection == HOME || (firstSelection != HOME && secondSelection != NONE))
        firstSelection = myOrganism[idx];
        secondSelection = NONE; 
    }
    else{
        secondSelection = myOrganism[idx];
        addNewOrganism(); 
    }
...
{
    var newOrgamism = findCombo(firstSelection, secondSelection);
    if(newOrgamism!=NONE)
    {
        if(myOrganism.indexOf(newOrgamism)<0)
        {
            myOrganism.push(newOrgamism);
        }
    }
}
...
{
    var idx = parseInt(organismForm.orgae1.value)-1; 
    if(idx<ORGANISM_OFFSET || idx >= myOrganism.length)
    {
        return; 
    }
    myOrganism.splice(idx, 1);
    organismForm.orgae1.value = ""; 
    display(); 
